...
opts.val = [] ;
opts.gpus = [] ;
opts.prefetch = true ;      % lph 2016/12/04 12:20  true-->false 2017/01/09 false-->true
opts.numEpochs = 300 ;
...
for epoch=start+1:opts.numEpochs
  ...
  % prepareGPUs(opts, mod(epoch - (start+1), 2)) ;
...
%   % lph 2016/11/11 5:50     % comment-2016/12/11 7:08
%   if mod((fix((t-1)/params.batchSize)+1), inf) == 0
%       % quit force;
%       custom_id = find(cellfun(@(x) strcmp(x.type, 'custom'), net.layers), 1, 'first');
%       if ~isempty(custom_id)
%           if isfield(net.layers{custom_id}, 'weights')
%               if strcmp(net.layers{custom_id}.regu_method, {'AF'})
%                   % [repmat('%f\t', 1, size(A, 2)) '\n']
%                   fprintf([repmat('%-+8.4f\t', 1, size(net.layers{custom_id}.weights{1}, 2)) '\n'], net.layers{custom_id}.weights{1}); fprintf('\n');
%               else
%                   if strcmp(net.layers{custom_id}.regu_method, {'Burg'})
%                       P = exp(net.layers{custom_id}.weights{1});
