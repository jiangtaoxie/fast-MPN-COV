...
#include "bits/nncov_traceNorm.hpp"
...
//          output.init(in[IN_DATA]) ;
     ...
     if (!backMode) {
		 if (!auxMode ){
             error = vl::nncov_traceNorm_forward(context,
                                                 output, 
                                                 data,
									             aux_T) ;
		 } else {
			 error = vl::nncov_traceNorm_aux_forward(context,
				                                     output,
													 data,
													 aux_T);
		 }
		 ...
		 if ( !auxMode ) {
             error = vl::nncov_traceNorm_backward(context,
                                                  derData, 
                                                  data, 
                                                  derOutput,
												  derOutput_aux,
			 ...
			 error = vl::nncov_traceNorm_aux_backward(context,
				                                      derData,
													  derData_aux,
													  data,
													  derOutput,
