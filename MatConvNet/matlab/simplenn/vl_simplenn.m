...
for i=1:n
  ...
  switch l.type
   ...
   case 'cov_pool'
    ...
    case 'cov_traceNorm'
        try isempty(res(i+1).dzdx{2})
        catch
            bs_num = size(res(i).x,4);
            type = class(res(i).x);
            dldx_tr = zeros(1,1,1,bs_num,type);
            res(i+1).dzdx{2} = dldx_tr;
        end
        res(i).dzdx = vl_nncov_traceNorm(res(i).x,res(i+1).aux{1}, res(i+1).dzdx{1},res(i+1).dzdx{2},cudnn{:});
     case 'cov_froNorm'
        try isempty(res(i+1).dzdx{2})
        catch
            bs_num = size(res(i).x,4);
        ...
        res(i).dzdx = vl_nncov_froNorm(res(i).x,res(i+1).aux{1},res(i+1).dzdx{1},res(i+1).dzdx{2},cudnn{:});
    case 'cov_sqrtm'
        res(i).dzdx{1} = vl_nncov_sqrtm(res(i).x,...
                                     res(i+1).aux{1},...
                                     res(i+1).aux{2},...
                                     res(i+1).dzdx,...
                                     'coef',l.coef,...
                                     'iterNum',l.iterNum,...
                                     cudnn{:});
     case 'cov_traceNorm_aux'
          [res(i).dzdx, res(i-1).dzdx{2}] = vl_nncov_traceNorm(res(i).x,res(i+1).aux{1}, res(i+1).dzdx,cudnn{:});
     case 'cov_froNorm_aux'
          [res(i).dzdx, res(i-1).dzdx{2}] = vl_nncov_froNorm(res(i).x, res(i+1).aux{1},res(i+1).dzdx,cudnn{:});
